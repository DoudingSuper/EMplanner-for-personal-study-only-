...
bool plotKappa(std::vector<waypoint> &vecTraj, const std::string& format = "")
{
    std::vector<double> x,y;
    for(auto & i : vecTraj)
    {
        x.push_back(i.x);
        y.push_back(i.dir * 100);
    }
    return plot(x,y,format);
}

bool plotTrajectory(std::vector<obstacle> &vecTraj, const std::string& format = "")
{
    std::vector<double> x,y;
    for(auto & i : vecTraj)
    {
